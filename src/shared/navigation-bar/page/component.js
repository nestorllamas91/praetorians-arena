...
import styles from '@/shared/navigation-bar/page/styles';
...
export default function Page(props) {
  ...
  async function checkInscriptionPeriod() {
    try {
      ...
      const url = `/api/competitions/competitions/${nextSeason}`;
  ...
  async function handleSeason(year, quarter) {
    try {
      const url1 = `/api/competitions/competitions/${year}-q${quarter}`;
      const res1 = await axios.get(url1);
      const firstCompetition = res1.data.output.data[0].competition;
      const firstTypeCompetition = firstCompetition.split('-')[0];
      const url2 = `/api/competitions/ranking/${year}-q${quarter}/${firstCompetition}`;
      const res2 = await axios.get(url2);
      if (player.isAuthenticated && firstTypeCompetition === 'dueling') {
        Router.push(`/competitions/${res2.data.output.data.rankingID}?player=true`);
      } else {
