...
import Game from '@/menu/games/game/component';
import styles, { stylesMaterialUI, stylesSyncfusion } from '@/menu/players/my-combats/my-combat/styles';
import { dateLocalToUtc, dateUtcToLocal, fixRankingSelectors } from '@/utils/functions';
...
function MyCombat(props) {
  ...
  async function getCombat() {
    try {
      const url = `/api/account/my-combats/combat/${combatId}`;
    ...
    MySwal.fire({
      ...
      if (result.value) {
        try {
          ...
          const url = '/api/account/my-combats/upload';
  ...
  function getSequenceData(decodedByteData, sequenceFile) {
    ...
    const { playfullDateIsoUtc, playDateUTC, playTimeUTC, playfullDateIsoLocal, playDateLocal, playTimeLocal } =
    ...
    const unknownMap = {
      ...
      numPlayers: 0,
      path: 'unknown',
      imagePath: '/assets/images/maps/unknown.png'
    };
    maps.map(map => {
      if (map.path === mapFilename) {
        sequenceData.gameData.map.id = map.id;
        sequenceData.gameData.map.name = map.name;
        sequenceData.gameData.map.numPlayers = map.numPlayers;
        sequenceData.gameData.map.path = map.path;
        sequenceData.gameData.map.imagePath = map.imagePath;
      }
    });
    if (sequenceData.gameData.map.id === undefined) {
      sequenceData.gameData.map.id = unknownMap.id;
      sequenceData.gameData.map.name = unknownMap.name;
      sequenceData.gameData.map.numPlayers = unknownMap.numPlayers;
      sequenceData.gameData.map.path = unknownMap.path;
      sequenceData.gameData.map.imagePath = unknownMap.imagePath;
    }
    foo = fread(decodedByteData, 3);
