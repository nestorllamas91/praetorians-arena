...
import styles, {
  stylesMaterialUI
} from '@/menu/players/my-inscriptions/open-rankings/open-rankings-tables/open-ranking/styles';
...
function OpenRanking(props) {
  ...
  async function checkPlayerInRanking() {
    try {
      const url = `/api/account/my-inscriptions/check-player/${props.ranking.rankingID}/${player.data.steamId}`;
  ...
  async function checkRankingMaxPlayers() {
    try {
      const url = `/api/account/my-inscriptions/check-max-players/${props.ranking.rankingID}`;
      const res = await axios.get(url);
      ...
      if (result.value) {
        try {
          if (props.ranking.numPlayersPerGame === 2 || props.ranking.numPlayersPerGame === 0) {
            const url = `/api/account/my-inscriptions/add-player/${props.ranking.rankingID}/${player.data.steamId}`;
